...
def read_input(i):
    with open(f'solutions/2020/{i}/input.txt', 'r') as f:
        return f.read()
...
def activate(cubes, x, y, z):
    cubes[x,y,z] = "A"
    cubes = extend(cubes, x, y, z)
    return cubes
    
def deactivate(cubes, x, y, z):
    cubes[x,y,z] = "I"
    cubes = extend(cubes, x, y, z)
    return cubes

def extend(cubes, x, y, z):
...
def iterate_over(cubes, z=None):
    for x in range(cubes['x'][0]-1,cubes['x'][1]+2):
        for y in range(cubes['y'][0]-1,cubes['y'][1]+2):
            if z is None:
                for zk in range(cubes['z'][0]-1,cubes['z'][1]+2):
                    yield (x,y,zk)
            else:
                yield (x,y)
